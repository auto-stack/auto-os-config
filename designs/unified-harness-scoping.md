...
### 1.2 第一阶段简化:只继承,不定制(2026-06-26 决定)

为降低复杂度,**第一阶段放弃"定制"(字段覆盖)**,只做**直接继承**:

- APP "选用"一个 OS 级 Harness = 原样继承,不改任何字段
- APP 自定义 Harness = 完全独立定义,内容格式与 OS 级**完全一致**

这样通用 Harness 和 APP Harness 的区别**纯粹是作用域**(定义在哪),配置内容一模一样。

| 操作 | 第一阶段 | 何时做 |
|---|---|---|
| **直接继承**(原样用 OS 级) | ✅ 本次 | — |
| **扩展**(APP 自定义新建) | ✅ 本次 | — |
| **定制**(inherit + 字段覆盖) | ⏸ 暂不做 | 需求涌现时,复用 Plan 004 的 `inherit:` |

> 这意味着第一阶段的"定制"如果真需要,只能通过"复制一份到 APP 级再改"实现(显式、无歧义)。

**导航重构**(本次同时做):侧栏从平级 5 项改为**可折叠分组**:
```
⚙️ AutoOS Settings
  [搜索]
  ── AI Daemon ──────────────  (平级,不可折叠)
  ...
  [主题色]
```
每个 APP(目前只 Auto Musk)的应用配置里,新增 **Harness 配置**两部分:
1. **选用 OS 级 Harness**:勾选清单(继承哪些通用 agents/skills/roles)
2. **APP 自定义 Harness**:自己定义(格式同 OS 级)
