...
> - **登记偏差/待办**（余量主要来源）：可编辑表格（css-era TableField：列头/单元格 select/+Row/×，现为只读 JSON 块）；tags/multiselect chips（现为加号式输入）；select 真下拉 + 枚举选项管线（现为自由文本）；删除确认用行内条（css-era 为 modal）；编辑提交时机 blur/回车（css-era 逐键 live-apply）；theme_picker ✓ 为文本勾（css-era svg）；vm 轨编辑器/集合视图因本轮 vue-first 重写需在 J1 解锁后按新形态重验（009 §0.2 顺延）。
