...
- [x] 收尾 workaround 审计（2026-08-25）：①修正 vm_collection 硬编码 `store.Open("roles")` → `.module_id`（当时误诊为 prop 读取崩溃，实为 VG16 方法名撞名；Modes/Harness Roles 实机复验通过）；②api.at 剪除调试残留（probeG1/G2、select_info、arr_len、first_id、fetch_mods_flat、entry_at/2）与按 VG13 违禁模式写就的死代码（表格族/嵌套返回族/未用块操作族，63→55 pub fn）；剪除后 vm 门禁复跑全绿
