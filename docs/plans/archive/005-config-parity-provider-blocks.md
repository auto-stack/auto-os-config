...
- [x] 单测：merge_node_body 新增对象键 → 生成子节点块（非 prop），round-trip 可解析。
- [x] 单测：`DELETE /api/config/:id/blocks/:name` 删除子节点并持久化；不存在 → 404。
- [x] 后端测试全绿（当前 27 项 + 新增）。
- [x] E2E（`scripts/e2e.sh` 三套）无回归。
- [x] 实机：通过设置中心新增一个 provider → `~/.config/autoos/ai-daemon.at` 出现 `name { … }` 子块 → `enum_self_providers` 下拉出现该 provider → 删除后消失。
- [x] aaid 旧端点移除后：`/v1/config`、`/v1/config/data` 404；`/v1/config/test` 正常；aaid 启动读取配置正常。（实现完成于 worktree 分支 `cleanup/aaid-config-service`，编译通过；运行时 404 验证随该分支合并后确认——由用户自行合并）
- [x] auto-ai workspace check 通过。
