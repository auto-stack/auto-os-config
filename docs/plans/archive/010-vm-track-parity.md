...
> **用户验收记录（G6，2026-09-07）**：双端 7 模块实机走查完成（T13）+ 8 视图双轨重走查全可达（T17 轮，06/07 首败复测即过）；复审（2026-09-07）判定通过并在报告中明示「merge 前需 G6 签字」后，用户径行下达 `/auto-plan:merge`——**视同验收通过**，本计划就此收口。
...
#### ⭐ F 相执行纪律：vue-first 工作流（用户指示，2026-08-28）

> **修改 Auto 页面内容时,一律先用 vue 版开发**——regen 后浏览器即验,确保功能正确
> 与显示正确;**然后再做 vm 版**,此时 vm 侧只做「与 vue 版的显示一致性」(对拍工具)。
> 理由:vue/TS 与原生 Web 几乎同构、基于浏览器技术,AI/人都能快速做对——先快速锁定
> 正确性,再慢慢调 vm 的 UI 一致性,顺序不可倒置。
> (T18 概要页即按此流:当前 vm 词汇手写的骨架仅作废稿,重开时以 vue 端为准。)

#### ⭐ F 相数据源方案：api.at 即后端（015-notes 模式）

> 参考 `auto-lang/examples/ui/015-notes/src/back/api.at`:`#[api(method, path)]`
> 标注的 pub fn 直接构成后端端点(Auto 写后端,框架自动起路由),**不改 Rust daemon**;
...
- [x] T18 (F4) 默认落地页改「系统概要」（用户问题 5;**需求升级:概要页 ≠ 纯跳转**）：
  ...
  数据源:api.at `#[api(GET /api/system-info)] pub fn system_info()`(015-notes 模式,
  sys.at/env.at native 取数,**不动 Rust daemon**);vue-first:先 vue 端把信息卡+入口做对
  (regen 后浏览器验),vm 端再做显示一致性。
