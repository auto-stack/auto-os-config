...
| 562迁移 | 已知限制 | **562 侧栏族迁移（ebf0076，直接提交无计划文件）漏更 vue e2e 锚**：nav 族退役后 nav-item/nav-name marker 与 data-active 契约锚消失，三套件选择器全红（该提交只验 regen/build/vm 冒烟）。012 复审已修（marker 按惰性测试钩子惯例补回 sidebar.at + theme 激活锚改 `[class*="bg-primary/10"]`）；**仍开放**：562 后侧栏视觉 parity 台账未重基线（012 的 2.1–5.0% 数字对应 nav 族形态已过期）。 | `auto/src/front/sidebar.at`、`scripts/track-parity/` |
