...
| 011 | 退役登记 | 旧 Rust daemon `backend/` 由 Auto 版外部 back `auto-os-config-back/` 取代(#[api] 契约 + Plan 061 cdylib 桥 + axum 双传输,端口沿用 17701)——源码归档 `archive/backend/`,e2e.sh 服务源已切换。活代码引用清零。 | `archive/backend/`、`auto-os-config-back/` |
| 007 | 已知限制 | **vm 轨 v1 偏差 → 009/010 终态**：侧栏分组折叠 ✓ 集合过滤 ✓ accent 持久化 ✓ 均已平（e2e-vm 断言在案）；字段顺序字母序已由 446 批三 D6 preserve_order 修复（**偏差登记已撤**，2026-08-29）；**仍开放**：select 控件 vm 渲染缺位、table thead 内置暗色——已定性为上游缺口（446 §P4/P5）。 | `docs/plans/010-*` 残差台账 |
...
| 013 | 已知限制 | **AutoTerm vue 轨未适配**：本计划验收面=vm 桌面（OS-013 §变更摘要）；registry 新增模块对双轨侧栏均可见，但 vue 部署树（regen 未跑）App.vue 无 autoterm_page 分发臂——vue 侧点击 AutoTerm 项无自定义视图（回退通用表单面）。**012 复审精确化（2026-09-07）**：app.at 真源已含 autoterm 臂/Tick 节拍，任何 regen 都会物化 App.vue+AutoTermPage.vue+useAutoTermStore.ts 三件，其中 store 对 `Term` 为无导入裸引用——vue-tsc 必红；收口需 Term web shim（或 regen 排除规则）+ app.at 新增臂的 vue codegen 兼容核验（`<terminal>` 标签 vue 侧未接线）。 | `auto/src/front/app.at`、`auto/gen/regen.sh` |
| 562迁移 | 已知限制 | **562 侧栏族迁移（ebf0076，直接提交无计划文件）漏更 vue e2e 锚**：nav 族退役后 nav-item/nav-name marker 与 data-active 契约锚消失，三套件选择器全红（该提交只验 regen/build/vm 冒烟）。012 复审已修（marker 按惰性测试钩子惯例补回 sidebar.at + theme 激活锚改 `[class*="bg-primary/10"]`）；~~parity 台账未重基线~~ → **已重基线**（010 T17 连带，2026-09-07：vue-vs-vm 8 视图新基线 00=6.04/01=7.59/02=2.75/03=2.77/04=3.73/05=3.66/06=3.00/07=2.13，见 010 §残差台账·重基线；00 涨幅主项=562 sidebar 族形态差+概要页改版，无结构性红块）。 | `auto/src/front/sidebar.at`、`scripts/track-parity/` |
| 012复审 | 已知限制 | **e2e-vm Test-connection 断言依赖 aaid 在线**：门禁只认 ok/fail，aaid（:17654）离线时 honest 终态 unreachable 判 FAIL（vue 侧门禁容忍离线，双轨口径不一致）；另 press('Test', 2000) 窗口偏紧偶发按钮未及渲染。非代码回归（干净 HEAD 对照同挂，aaid 拉起后全绿）。 | `scripts/e2e-vm.mjs` |
| 009复审 | 已知限制 | **N1 类串白名单 lint 未交付（obsolete by supersession）**：check-style-classes.mjs 从未落地，治理对象（手写 `*_vm.at` 类串）已随统一视图 + 012/562 契约组件化整体退役；若未来回退手写类串形态需重新评估 policing。 | `docs/plans/archive/009-*` 复审记录 |
| 009复审 | 已知限制 | **阶段一四项 vue 微残差（vs css-era）**：编辑器首存确认行内条（css-era 为 window.confirm）/ 编辑提交时机 blur·回车（css-era 逐键 live-apply）/ theme_picker ✓ 文本勾（css-era svg）/ modal 背板点击不关闭（.at 视图无法判 e.target）。头部补录在案、量级微小，正式登记以防静默。 | `docs/plans/archive/009-*` 复审记录 |
