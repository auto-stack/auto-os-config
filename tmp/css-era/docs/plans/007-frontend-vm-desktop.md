...
- [x] Phase 5：`scripts/e2e-vm.mjs` 9 断言两连绿（boot/导航/集合 list-select-edit-save/Test connection/主题），fixture 经 HTTP 还原零残留；auto/README vm 章节 + VG 清单 + 已知偏差重写；KNOWN-DEBT 登记（见下）；探针工程 tmp/vm-probes/ 随收尾退役（006 惯例，结论均已回填本文件）
- [x] 收尾 workaround 审计（2026-08-25）：①修正 vm_collection 硬编码 `store.Open("roles")` → `.module_id`（当时误诊为 prop 读取崩溃，实为 VG16 方法名撞名；Modes/Harness Roles 实机复验通过）；②api.at 剪除调试残留（probeG1/G2、select_info、arr_len、first_id、fetch_mods_flat、entry_at/2）与按 VG13 违禁模式写就的死代码（表格族/嵌套返回族/未用块操作族，63→55 pub fn）；剪除后 vm 门禁复跑全绿；③**修复静默 stale-deploy**：vm widgets 的 v-for 缺 `key:` 使 vue 严格模式 regen 一直失败，而 `regen.sh | tail` 管道吞掉退出码——此前数轮"vue e2e 绿"实为旧部署产物（含 store 改名从未部署）。修复：全部 vm widgets 的 for 补 `key:`（G8 同款 wrapper div）、regen.sh 排除 vm 专属组件的 web 部署（其 store 直连导入是 vue codegen 的错误路径，登记 KNOWN-DEBT）、api.ts 补齐 `fieldDisplayOf/editTagField` 契约函数；双轨门禁在真部署态复跑全绿（vue 28 断言 + vm 9 断言）。**流程教训：regen 结果必须显式检查退出码/输出，勿经管道**
- [x] 终态：vue web 版与 vm 桌面版并存，`./scripts/e2e.sh` 与 `node scripts/e2e-vm.mjs` 双绿为仓库门禁
