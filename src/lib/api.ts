// src/back/api.ts — the web implementation of the back.api surface (Plan 006 D2;
// Plan 559 W2 made this the checked-in source of truth: auto build installs it
// into the gen tree as src/lib/api.ts, and regen.sh mirrors it to the host).
//
...
// ── inlined from host src/editor/types.ts (Plan 002 inference engine) ──
// Plan 559 W2: the twin must be self-contained — it compiles standalone
// in the gen tree (gen/front/vue/src/lib/api.ts) where ../editor/types
// does not exist. Keep byte-compatible with the host file.
...
function pickModule(m: any) {
  return {
    ...
    group: m.group ?? '',
    format: m.format ?? '',
    // Plan 551: 插件自定义 UI 声明——与 back 桩同步(view 是 .at 文法
    // 关键字,内部字段名 view_name;widgets 保留对象映射)。
    view_name: m.view ?? '',
    widgets: m.widgets ?? '',
  }
}
...
// ── 2026-08-27 像素对拍二阶段：select 选项 / tags chips / 可编辑表格 ──────
...
/** multiselect 复选框勾选态平行数组（视图无 contains，api 预计算）。 */
...
export async function warmEnumsText(body: unknown, moduleId: string): Promise<boolean> {
  const obj = asBodyObj(body) ?? {}
...
// ── Plan 559 W2: desktop-cfg / wallpaper surface ───────────────────────────
// Ported from back/api.at (vm track) — the same `use back.api` names the
// generated useDesktopCfgStore / WallpaperPicker import. Text-contract shapes
// mirror the vm implementation: JSON bodies travel as strings.

/** Fail-soft desktop module config fetch → { ok, error, body (JSON text) }.
 * Wraps fetchConfigSafe into the { ok, error, body } shape the vm fn returns. */
export async function fetchDesktopCfgSafe(
  id: string,
): Promise<{ ok: boolean; error: string; body: string }> {
  const r = await fetchConfigSafe(id)
  if (!r.ok) return { ok: false, error: r.error, body: '{}' }
  return { ok: true, error: '', body: r.value }
}

/** Flat field read off a JSON body text (unquoted display value; '' if absent). */
export function cfgField(body: string, key: string): string {
  try {
...
/** Fail-soft POST /api/action/list-dir → { ok, error, items (JSON text), count }. */
export async function listImagesSafe(
  dir: string,
): Promise<{ ok: boolean; error: string; items: string; count: number }> {
  const fail = { ok: false, error: '目录枚举失败', items: '[]', count: 0 }
  try {
    const resp = await fetch('/api/action/list-dir', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    return { ok: true, error: '', items: text, count: parsed.length }
  } catch {
    return fail
  }
}

/** Entry count of a list-dir items JSON text. */
export function imageCount(items: string): number {
  try {
...
/** Entry of a list-dir items JSON text → { name, path } display strings. */
export function imageAt(items: string, i: number): { name: string; path: string } {
  const un = (v: unknown): string =>
    typeof v === 'string' ? v : v === undefined || v === null ? '' : JSON.stringify(v)
  try {
    const parsed = JSON.parse(items)
    const ent = Array.isArray(parsed) ? parsed[i] : null
    return { name: un(ent?.name), path: un(ent?.path) }
  } catch {
    return { name: '', path: '' }
  }
}
