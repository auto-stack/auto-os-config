...
function SelectOverview(ri: any): void {
  store.Overview();
...
<template>
    <aside class="sidebar flex flex-col w-[280px] h-full shrink-0 bg-card border-r border-border rounded-b-2xl">
      <div class="w-full min-h-0 flex-1 flex flex-col">
        ...
        <div class="nav-list flex-1 overflow-auto px-2 pt-2 flex flex-col">
          <template v-if="store.search == ''">
            <div v-for="(m, ri) in store.view_standalone" :key="m.id">
              <div>
