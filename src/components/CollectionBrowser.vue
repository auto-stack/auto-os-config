...
function ApplyEntry(p: any): void {
  let k = p;
...
async function Toggle(p: any): Promise<void> {
  let k = p;
  let nv: string = 'false';
          ...
          <template v-if="store.loading == false">
            ...
            <template v-if="store.is_read_only == false">
              <div class="flex flex-col gap-4 fields gap-1">
                <div v-for="e in store.entries" :key="e.key">
                  <template v-if="e.is_table">
                    <div class="flex flex-col gap-4 field-row border rounded p-2 gap-1 bg-white">
                      <span class="field-label text-sm font-medium text-[#616161]" :key="e.key">{{ e.label }}</span>
                  ...
                  <template v-if="e.kind == 'toggle'">
                    <div class="flex flex-row gap-4 field-row items-center gap-3 py-2">
                      <span class="field-label text-sm font-medium text-[#616161] w-[160px] shrink-0" :key="e.key">{{ e.label }}</span>
                      <input type="checkbox" :checked="e.is_on" :id="e.key" @click="Toggle(e.key)" />
                  ...
                  <template v-if="e.kind == 'number'">
                    <div class="flex flex-row gap-4 field-row items-center gap-3 py-2">
                      ...
                      <button class="btn px-3 py-1 text-xs rounded border border-[#e0e0e0] bg-white" @click="ApplyEntry(e.key)">Apply</button>
                    </div>
                  </template>
                  ...
                  <template v-if="e.kind == 'tags'">
                    <div class="flex flex-col gap-4 field-row py-2 gap-1">
                      <div class="flex flex-row gap-4 items-center gap-3">
                        ...
                        <button class="btn px-3 py-1 text-xs rounded border border-[#e0e0e0] bg-white" @click="TagAdd(e.key)">Add</button>
                      </div>
